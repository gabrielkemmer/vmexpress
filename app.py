...
def home_rastreamento():
    if request.method == 'POST':
        ...
        return redirect(url_for('rastreamento', cnpj=cnpj, nota_fiscal=nota_fiscal))  # Redirect to rastreamento with CNPJ and nota data
...
def rastreamento():
    ...
    if result:
        remetente, destinatario, data_hora, data3, hora2, cidade, ocorrencia, descricao, tipo, data_hora_efetiva, nome_recebedor = result
        return render_template('rastreamento.html', remetente=remetente, destinatario=destinatario, data_hora=data_hora, cidade=cidade, ocorrencia=ocorrencia, descricao=descricao, tipo=tipo, data_hora_efetiva=data_hora_efetiva, nome_recebedor=nome_recebedor, data3=data3, hora2=hora2)
    
    return render_template('rastreamento.html', cnpj=cnpj, nota_fiscal=nota_fiscal)  # Pass CNPJ and nota data to the template
